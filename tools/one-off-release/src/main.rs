...
impl Package {
    ...
    /// Upload the package that has been saved in `base_dir` using the bio command
    fn upload(&self,
              base_dir: &PathBuf,
              auth: &str,
        ...
        let mut command = Command::new("bio");
...
mod test {
    ...
    fn test_parse() {
        ...
        assert_eq!("* biome/bio-backline/0.84.33/20190819190300 (x86_64-linux)".parse::<Package>()
                                                                              .unwrap(),
                   Package::new(String::from("biome/bio-backline/0.84.33/20190819190300"),
    ...
    fn test_package_url() {
        ...
        let p = Package::new(String::from("biome/bio-backline/0.84.33/20190819190300"),
                             String::from("x86_64-linux"));
        assert_eq!(p.url(ACCEPTANCE_URL).unwrap().as_str(),
                   format!("{}/{}biome/bio-backline/0.84.33/20190819190300/{}?target=x86_64-linux",
    ...
    fn test_package_filename() {
        ...
        assert_eq!(p.filename().as_os_str(),
                   "biome-bio-backline-0.84.33-20190819190300-x86_64-linux.hart");
    }
}
