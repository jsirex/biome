...
promote_from_one_channel_to_another() {
    ...
    package_name="${2}" # e.g. "bio-launcher"
    ...
    ${bio_binary} pkg promote --auth="${HAB_AUTH_TOKEN}" "${artifact}" "${to_channel}" "${target}"
...
case "${launcher_action}" in
    ...
    "use-stable-launcher")
        ...
        for target in "${launcher_platforms[@]}"; do
            promote_from_one_channel_to_another "${target}" "bio-launcher" "stable" "${release_channel}"
        done
