...
Write-Host "--- Installing latest stable Biome binary"
$baseHabExe = [HabShared]::install_base_biome_binary("latest", "stable")
Write-Host "--- Using bio executable at $baseHabExe"
...
Push-Location "C:\build"
    ...
    Write-Host "--- Running bio pkg build for $Component"
    Invoke-Expression "$baseHabExe pkg build components\$Component --keys core"
    . "results\last_build.ps1"

    Write-Host "Running bio pkg upload for $Component to channel $ReleaseChannel"
    ...
    If ($Component -eq 'bio') {
        ...
        Invoke-Expression "buildkite-agent meta-data set 'bio-version-x86_64-windows' '$pkg_ident'"
        Invoke-Expression "buildkite-agent meta-data set 'bio-release-x86_64-windows' '$pkg_release'"
        Invoke-Expression "buildkite-agent meta-data set 'bio-artifact-x86_64-windows' '$pkg_artifact'"
