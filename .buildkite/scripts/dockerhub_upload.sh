...
# Build the bio base image and the latest studio image for the given release channel.
# This results in something like:
# biome/default-studio:0.61.0
...
image_name="biome/default-studio-${target}"
image_name_with_tag="${image_name}:${version}"

if is_fake_release; then
  image_name_with_tag="biome/fakey-mc-fake-face-studio"
fi
...
docker build --build-arg PACKAGE_TARGET="${target}" -t "biome-${target}:bio-base" .
docker build \
