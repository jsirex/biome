...
param (
  ...
  # The docker image name. Defaults to "biome/win-studio-x86_64-windows
  [string]$imageName = "biome/win-studio-x86_64-windows",
...
try {
    ...
    Write-Host "Installing and extracting initial Biome packages"
    $InstallHarts = @(
        "biome/bio-studio",
        "biome/bio-sup",
    ...
    $InstallHarts | % {
        Invoke-Expression "bio pkg install $_ --channel=$ReleaseChannel"
        if ($LASTEXITCODE -ne 0) {
            Write-Error "bio install failed for $_, aborting"
        }
    }
    $studioPath = bio pkg path biome/bio-studio
    if ($LASTEXITCODE -ne 0) {
      Write-Error "biome/bio-studio must be installed, aborting"
    }
    
    Write-Host "Purging container bio cache"
...
# Install the biome windows service and then allow color and strip the log timestamps.
...
RUN `$env:HAB_LICENSE='accept-no-persist'; ``
    ...
    &/hab/pkgs/$ident/bin/bio/bio.exe pkg exec core/windows-service install; ``
...
ENTRYPOINT ["/hab/pkgs/$ident/bin/powershell/pwsh.exe", "-ExecutionPolicy", "bypass", "-NoLogo", "-file", "/hab/pkgs/$ident/bin/bio-studio.ps1"]
