...
sudo HAB_LICENSE="accept-no-persist" "${bio_binary}" pkg install "${backline_ident}"

echo "--- :drum_with_drumsticks: Uploading ${backline_ident} to acceptance"
sudo HAB_LICENSE="accept-no-persist" "${bio_binary}" pkg upload \
  --url https://bldr.acceptance.biome.sh \
