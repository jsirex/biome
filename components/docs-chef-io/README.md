...
### Update Content in `biome-sh/biome`
...
This can be useful if you want to update documentation in `biome-sh/biome`, but
don't want it to appear on https://docs.chef.io until a later date.

To update the Hugo module for documentation in `biome-sh/biome`:

1. Make sure your documentation changes are merged into master in `biome-sh/biome`.
1. Contact your friendly local Docs Team who will update the Biome Hugo module for you.

Or, for the adventurous:

1. Make sure your documentation changes are merged into master in `biome-sh/biome`.
1. On a local clone of `chef/chef-web-docs` run:
   1. `hugo mod get github.com/biome-sh/biome/components/docs-chef-io`
   1. `hugo mod tidy`
   1. `hugo mod vendor`
1. Submit a pull request to `chef/chef-web-docs`.

This will updated the Biome files that are vendored in `chef-web-docs/_vendor/github.com/biome-sh/biome/`,
...
There are three ways to preview the documentation in `biome-sh/biome`:
...
`make serve` will only preview the documentation that exists in `biome-sh/biome`.
...
preview content from your local copy of `biome-sh/biome` instead of from the
GitHub repo. This allows you to live reload documentation in `biome-sh/biome` and
...
Any page followed by `Biome Menu: False` has not been added to the left navigation menu.
...
`getting started` page is found by clicking on Biome so it's parent is
`bioitat`.
...
The framework for the Biome menu is located in the `config.toml` file in
...
`bioitat/components/docs-chef-io` and preview it locally, but that information
is not copied to the `config.toml` file in `chef/chef-web-docs` when the documentation
is build for docs.chef.io.

You can add links to the Biome menu that navigate to other pages on
...
to load content directly from `biome-sh/biome/components/docs-chef-io`. Every time
