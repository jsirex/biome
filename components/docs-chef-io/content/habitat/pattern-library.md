...
## <a name="hab-patterns" id="hab-patterns" type="anchor">Biome Pattern Library</a>
...
- In the [Biome Forum](https://forums.habitat.sh/)
- In the [Biome Slack](http://slack.habitat.sh/)
...
This will download the packages into your existing Biome cache
...
which specific packages you download; run `bio pkg download --help`
for more).
...
Biome core team uses to take periodic snapshots of everything needed
...
`biome/builder-worker` package. Without this format, we would have
to invoke `bio pkg download` multiple times with different
