...
[menu]
  [menu.bioitat]
    title = "Plan Overview"
    identifier = "habitat/plans/plans-overview Introducing Biome Plans"
...
As shown in an example above, there are occasions when you want to override the default behavior of the bio-plan-build script. The Plan syntax guide lists the default implementations for [build phase callbacks](/docs/reference/#reference-callbacks), but if you need to reference specific packages in the process of building your applications or services, then you need to override the default implementations as in the example below.
...
When overriding any callbacks, you may use any of the variables, settings, or functions in the [Plan syntax guide](/docs/reference/), except for the runtime template data. Those can only be used in Application Lifecycle hooks once a Biome service is running.
...
You can use any of the [runtime configuration settings](/docs/reference/#template-data), either defined by you in your config file, or defined by Biome.
