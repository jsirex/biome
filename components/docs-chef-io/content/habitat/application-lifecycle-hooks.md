...
> **Important** You cannot block the thread in a hook unless it is in the `run` hook. Never call `bio` or `sleep` in a hook that is not the `run` hook.

## Runtime Settings

[Biome's runtime configuration settings](/docs/reference/template-data) can be used in any of the plan hooks and also in any templatized configuration file for your application or service.

> **Note**: In Biome 0.68.0 and less, some hooks used `_` in their names. This is now deprecated; if you used them, please use `-` instead.
