...
#
# ```
# $ bio-pkg-mesosize [PKG ...]
# ```
#
# # Synopsis
#
# Create a Mesos application from a set of Biome packages.
#
...
Biome Package Mesosize - Create a Mesos application from a set of Biome packages
...
ARGS:
    <PKG_IDENT>      Biome package identifier (ex: acme/redis)
"
}
...
parse_options() {
  ...
  if [ "$PKG" == "unknown" ]; then
    print_help
    exit_with "You must specify one or more Biome packages to Mesosize." 1
  fi
}

# Create a bio studio baseimage and populate it with the application
build_tarball_image() {
  TARBALL_CONTEXT="$($_mktemp_cmd -t -d "${program}-XXXX")"
  pushd "$TARBALL_CONTEXT" > /dev/null
  env PKGS="$PKG" NO_MOUNT=1 bio studio -r "$TARBALL_CONTEXT" -t bare new
  echo "$PKG" > "$TARBALL_CONTEXT"/.bio_pkg
  popd > /dev/null
