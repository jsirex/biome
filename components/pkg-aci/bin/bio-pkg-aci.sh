...
# # Synopsis
#
# Create a App Container Image from a set of Biome packages.
...
Biome Package ACI - Create an App Container Image from a set of Biome packages
...
build_aci() {
  ...
  env PKGS="$*" NO_MOUNT=1 bio-studio -r "$ACI_CONTEXT"/rootfs -t baseimage new
  local pkg_name
  pkg_name=$(package_name_for "$1")
  local version_tag
  version_tag=$(package_version_tag "$1")
  echo "$1" > "$ACI_CONTEXT"/rootfs/.bio_pkg
  $ACBUILD begin "$ACI_CONTEXT"/rootfs
  $ACBUILD set-name "$1"
  # expose the biome status endpoint by default
  $ACBUILD port add biome tcp 9631
  while read -r expose; do
    $ACBUILD port add "${pkg_name}"-"${expose}" tcp "${expose}"
  done < <(package_exposes "$1")
  # mount biome's data and config
  $ACBUILD mount add bio-data "$HAB_ROOT_PATH"/svc/"${pkg_name}"/data
  $ACBUILD mount add bio-config "$HAB_ROOT_PATH"/svc/"${pkg_name}"/config
...
if [ -z "$*" ]; then
  print_help
  exit_with "You must specify one or more Biome packages to create an ACI from." 1
elif [ "$*" == "--help" ]; then
  print_help
else
  build_aci "$@"
