...
Biome Package CFize - Create a Cloud Foundry ready Docker image from a given package.
...
. ~/render.sh
mv ~/user.toml /hab/svc/$(basename "${bio_package}")/user.toml
exec /init.sh "\$@"
EOT
...
RUN bio pkg install core/jq-static
ADD cf-init.sh /
ADD helpers.sh /
ADD config.toml /
...
program=bio-pkg-export-cf

if [ "$#" -eq 0 ]; then
  print_help
  exit_with "You must specify 1 Biome package to CFize." 1
elif [ "$1" == "--help" ]; then
  print_help
elif [ "$#" -ne 2 ]; then
  print_help
