...
#
# ```sh
# $ publish-bio [FLAGS] [OPTIONS] <PKG_IDENT_OR_ARTIFACT>
# ```
#
...
COMMON FLAGS:
    ...
    -o  Specify the Bintray organization to publish to (default: biome)
    -r  Specify the Bintray repo to publish to (default: stable)
    -s  Skip publishing the artifact (upload only)
    -V  Prints version information

ARGS:
    <HART_FILE> A path to a local Biome artifact
"
}
...
# * `$_bio_cmd` (bio CLI for signing artifacts)
...
_build_slim_release() {
  info "Extracting Biome package $target_hart"
  ...
  # hab/pkgs/biome/bio/$VERSION/$RELEASE portion of the path, because
  ...
  if [[ $(find "${extract_dir}" \( -name bio -or -name bio.exe \) -type f | wc -l) -ne 1 ]]; then
    exit_with "$target_hart did not contain a \`bio' binary" 2
  fi

  local bio_binary
  bio_binary="$(find "$extract_dir" \( -name bio -or -name bio.exe \) -type f)"
  ...
  archive_name="bio-$(echo "$pkg_ident" | cut -d '/' -f 3-4 | tr '/' '-')-$pkg_target"
  build_dir="$tmp_root/build"
  pkg_dir="$build_dir/${archive_name}"

  info "Copying $bio_binary to $(basename "$pkg_dir")"
  mkdir -p "$pkg_dir"
  mkdir -p "$start_dir/results"

  if [[ $pkg_target == *"windows" ]]; then
    for file in "$(dirname "$bio_binary")"/*; do 
      cp -p "$file" "$pkg_dir/"
    done
  else
    cp -p "$bio_binary" "$pkg_dir/$(basename "$bio_binary")"
  fi

  info "Compressing \`bio' binary"
...
_publish_slim_release() {
  ...
  if $_jfrog_cmd bt package-show \
    ...
    $_jfrog_cmd bt package-create \
      ...
      --pub-dn=false \
      --pub-stats=false \
      --website-url=https://www.biome.sh \
