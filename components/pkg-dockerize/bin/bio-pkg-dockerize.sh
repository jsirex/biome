...
#
# ```
# $ bio-pkg-dockerize [PKG ...]
# ```
#
# # Synopsis
#
# Create a Docker container from a set of Biome packages.
#
...
Biome Package Dockerize - Create a Docker container from a set of Biome packages
...
docker_image() {
  env PKGS="$*" NO_MOUNT=1 bio-studio -r "$DOCKER_CONTEXT"/rootfs -t baseimage new
  local pkg_name
  ...
  echo "$1" > "$DOCKER_CONTEXT"/rootfs/.bio_pkg
...
if [ -z "$*" ]; then
  print_help
  exit_with "You must specify one or more Biome packages to Dockerize." 1
elif [ "$*" == "--help" ]; then
  print_help
else
  build_docker_image "$@"
