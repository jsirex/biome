//! bio command line interface analytics module.
//!
//! The `bio` command-line tool will optionally send anonymous usage data to Biome's Google
//! Analytics account. This is a strictly opt-in activity and no tracking will occur unless you
//! respond affirmatively to the question during `bio setup`. If you do not use `bio setup`, no
//! data will ever be sent.
//!
//! We collect this data to help improve Biome's user experience: for example, to know what tasks
...
//! and you typed `bio pkg build -k yourkey yourname/yourapp`, the fact that you were performing
...
//! transmitted.
//!
//! Please do not hesitate to [contact us](mailto:support@biome.sh) if you have questions or
//! concerns about the use of Google Analytics within the Biome product.
//!
...
//! root user and `$HOME/.bio/analytics/CLIENT_ID` when invoked by a non-root user). This is not
//! intended to track individual users or systems, but rather show patterns of usage in aggregate.
//! For example: "In general, users who generally start with `bio studio enter` tend to migrate to
//! using `bio pkg build` over time".
//!
...
//! program the value is currently hardcoded as `"cli--bio"`.
//!
...
//! `"bio--studio--enter"` or `"bio--artifact--upload"`. For CLI errors (where `"ec=clierror"`),
...
fn send_event(payload: &str) -> bool {
    // Create a new Hyper HTTP client, using a helper from the `biome_http_client` crate. This
...
fn client_id() -> String {
    ...
    // helper function from the `biome_core` crate. The `None` tells the function that there is
