...
mod inner {
    ...
    const EXPORT_PKG_IDENT: &str = "biome/bio-pkg-export-tar";
...
mod inner {
    ...
    pub async fn start(ui: &mut UI, _args: &[OsString]) -> Result<()> {
        let cmd = EXPORT_CMD.replace("bio", "").replace("-", " ");
        ui.warn(format!("Running 'bio {}' on this operating system is not yet supported. Try \
