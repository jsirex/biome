...
pub fn start(ui: &mut UI, cache_path: &Path, analytics_path: &Path) -> Result<()> {
    ...
    if ask_default_ctl_secret(ui)? {
        ui.br()?;
        ui.para("Enter your Biome Supervisor CtlGateway secret.")?;
    ...
    ui.para("The `bio` command-line tool will optionally send anonymous usage data to Biome's \
    ...
    ui.para("We collect this data to help improve Biome's user experience. For example, we \
...
fn ask_default_auth_token(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no("Set up a default Biome personal access token?",
                        Some(true))?)
}

fn ask_default_ctl_secret(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no("Set up a default Biome Supervisor CtlGateway secret?",
                        Some(true))?)
}
...
fn prompt_auth_token(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask("Biome personal access token",
...
fn prompt_ctl_secret(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask("Biome Supervisor CtlGateway secret",
                     default.as_ref().map(|x| &**x))?)
}
