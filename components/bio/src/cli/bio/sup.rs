...
pub struct SupRun {
    ...
    /// The name of the ring used by the Supervisor when running with wire encryption. (ex: bio sup
    /// run --ring myring)
    #[structopt(name = "RING",
                long = "ring",
                short = "r",
    ...
    /// system. Use the corresponding environment variable instead.) (ex: bio sup run --ring-key
    ...
    /// Load the given Biome package as part of the Supervisor startup specified by a package
    /// identifier (ex: core/redis) or filepath to a Biome Artifact (ex:
