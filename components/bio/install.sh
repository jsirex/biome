...
download_packages_chef_io_archive() {
  ...
  if [ "$_version" == "latest" ]; then
    url="${pcio_root}/${_channel}/biome/latest/bio-${_target}.${ext}"
  else 
    ...
    url="${pcio_root}/biome/${_version}/bio-${_target}.${ext}"
  fi
  
  dl_file "${url}" "${workdir}/bio-${_version}.${ext}"
  dl_file "${url}.sha256sum" "${workdir}/bio-${_version}.${ext}.sha256sum"

  archive="bio-${_target}.${ext}"
  sha_file="bio-${_target}.${ext}.sha256sum"

  mv -v "${workdir}/bio-${_version}.${ext}" "${archive}"
  mv -v "${workdir}/bio-${_version}.${ext}.sha256sum" "${sha_file}"
