...
do_begin() {
  # Set the real bio component directory as the "root" of this plan.
...
do_prepare() {
  ...
  la_ldflags="$la_ldflags -L$(brew --prefix "$formulas"/bio-libiconv.rb)/lib -liconv"

  export LIBARCHIVE_LIB_DIR=$(brew --prefix "$formulas"/bio-libarchive.rb)/lib
  export LIBARCHIVE_INCLUDE_DIR=$(brew --prefix "$formulas"/bio-libarchive.rb)/include
