...
if [[ ! -f /usr/local/bin/bio ]]; then
  info "Biome CLI missing, attempting to install latest release"
...
while true; do
  ...
  if [[ $(find /hab/cache/keys/ -name "core-*.sig.key") ]]; then
    break
  elif [[ -f /tmp/bio.sig.key ]]; then
    bio origin key import < /tmp/bio.sig.key
    rm -f /tmp/bio.sig.key
  else
    bio origin key import <<< "${ORIGIN_KEY}"
  fi
done
...
# Homebrew packages required to run `bio-plan-build.sh
install_if_missing coreutils
install_if_missing gnu-tar
install_if_missing wget
install_if_missing bash
install_if_missing bio-rq "$(dirname "$0")"/homebrew/bio-rq.rb

# Homebrew packages required to build `bio`
...
install_if_missing bio-libiconv "$(dirname "$0")"/homebrew/bio-libiconv.rb
install_if_missing bio-libarchive "$(dirname "$0")"/homebrew/bio-libarchive.rb
...
program="$(dirname "$0")/../../../plan-build/bin/bio-plan-build.sh"
