...
Function Get-Version($version, $channel) {
    ...
    if(!$version) {
        ...
        Get-File "https://api.bintray.com/packages/biome/$channel/bio-x86_64-windows" $jsonFile
...
Function Get-Archive($channel, $version) {
    $url = "https://api.bintray.com/content/biome/$channel/windows/x86_64/bio-$version-x86_64-windows.zip"
    $query = "?bt_package=bio-x86_64-windows"

    $bio_url="$url$query"
    $sha_url="$url.sha256sum$query"
    $bio_dest = (Join-Path (Get-WorkDir) "hab.zip")
...
Function Install-Biome {
    ...
    $folder = (Get-ChildItem (Join-Path (Get-WorkDir) "bio-*"))
