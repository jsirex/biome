...
if command -v bio > /dev/null 2>&1; then
  if bio pkg exec core/postgresql psql -U hab -q -c "COPY (SELECT * FROM shard_0.account_creation_report('$1')) TO '$output' WITH (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *)" builder_sessionsrv; then
  ...
  echo "Biome not installed. Aborting."
  exit 1
fi
