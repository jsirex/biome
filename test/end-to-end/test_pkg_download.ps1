...
Describe "bio pkg download" {
    ...
    It "'bio pkg download --channel stable --download-directory $cacheDir core/gzip' succeeds" {
        bio pkg download --channel stable --download-directory $cacheDir core/gzip
        Test-GzipIdents
    }
    It "'bio pkg download --channel stable --download-directory $cacheDir --file $identFile' succeeds" {
        Set-Content $identFile -Value "core/gzip"
        bio pkg download --channel stable --download-directory $cacheDir --file $identFile
        Test-GzipIdents
    }
    It "'bio pkg download --channel stable --download-directory $cacheDir --file $identFile' succeeds with comments and empty lines" {
...
"@
    ...
    It "'bio pkg download --channel stable --download-directory $cacheDir core/rust --target=x86_64-windows' succeeds" {
        bio pkg download --channel stable --download-directory $cacheDir core/rust --target=x86_64-windows
        Test-RustIdents
    }
