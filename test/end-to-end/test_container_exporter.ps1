...
function New-Image() {
    ...
    Write-Host (bio pkg export container core/nginx --tag-custom=$tag $extra_args | Out-String)
    "core/nginx:$tag"
}
...
Describe "Old 'bio pkg export docker' alias" {
    BeforeAll {
        $tag = New-CustomTag
        Write-Host (bio pkg export docker core/nginx --tag-custom=$tag | Out-String)
